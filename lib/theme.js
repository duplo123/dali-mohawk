...
import { resolveSkinPreset, DEFAULT_SKIN_KEY } from "../config/skin-presets.js";
...
export async function getTheme(huntSlug = DEFAULT_HUNT_SLUG) {
  ...
  if (huntSlug === DEFAULT_HUNT_SLUG) return expandPresets(defaultTheme);
  return null;
}
...
// Built hunts store fontPreset / skinPreset / voicePreset keys; the renderer
// expects fully-expanded style.fontStylesheetUrl, style.fonts, and
// style.skinCss. Expand on read so upstream callers stay preset-agnostic.
//
// skinPreset defaults to DEFAULT_SKIN_KEY ("parchment") when missing — that
// way the file-backed Mohawk theme renders as parchment without needing a
// new field added to config/theme.js.
function expandPresets(theme) {
  ...
  if (theme.fontPreset) {
    ...
    if (fontFields) {
      style.fontStylesheetUrl = fontFields.fontStylesheetUrl;
      style.fonts = fontFields.fonts;
    }
  }

  const skinKey = theme.skinPreset || DEFAULT_SKIN_KEY;
  const skinFields = resolveSkinPreset(skinKey);
  if (skinFields) {
    style.skinCss = skinFields.css;
  }
